...
def normalize1Darrayx(data):
    ...
    '''
    data = np.array(data)
    zero_array = np.zeros((data.shape))
...
def k_nearestNeigbours_sklearn(points3D_xy, points3D_z, interestPoint, k, leafSize = 30):
    ...
    ind = np.asarray(kng[1][0])    #ind(1 x N array): Contains the indices to neighbouring points returned. N is the size or number of neighbouring points returned.
...
def getCustom_xyValuesV2(N, min3d, max3d):
    x_vals = np.arange(N) # N=512, so i would be a 1D array like so: [0,1,2,3,4...509,510,511] 
    y_vals = np.arange(N) # N=512, so i would be a 1D array like so: [0,1,2,3,4...509,510,511]
...
def convert3Dxyz_2Dxy_zIntensity_all(database_path, outdir, P, N = 128, k = 10, sample_P_points = False):
    ...
    for file in files:
        ...
        # '''FLIP [x,y,z]'''
        pointcloud = pointcloud[:5000,:]
        xx = pointcloud[:, 0]
        yy = pointcloud[:, 2]
        zz = pointcloud[:, 1]
        pointcloud = np.column_stack((xx, yy, zz))
        #print("\nnpointcloudDDD:\n", pointcloud[:5])
        ...
        xVals = np.linspace(min_xyz[cvv], max_xyz[cvv], N)
        yVals = np.linspace(min_xyz[cvv], max_xyz[cvv], N)
        ...
        print('xyValues Size: ', np.asarray(xyValues).shape)


        # Ensure that xyValues is a proper Numpy array!
        
        xyValues = np.asarray(xyValues)
        xyValues = xyValues[:5000,:] 
        print("\nxyValues Data:\n", xyValues)
        ...
        min2d = np.min(xyValues, 0)
        max2d = np.max(xyValues, 0)
        min3d = np.min(pointcloud_xy, 0)
        max3d = np.max(pointcloud_xy, 0)
        print("\nmin2d:\t", min2d, "\nmax2d:\t", max2d, "\n\nmin3d:\t", min3d, "\nmax3d:\t", max3d)

        # Call INTENSITY computation function
        s1 =  time.time()
        #I2 = normalize1Darrayx(compute_INTENSITYv2(xyValues, pointcloud_xy, pointcloud[:, 2], k))
        I2 = normalize1Darrayx(compute_INTENSITYv3(xyValues, pointcloud_xy, pointcloud[:, 2], k))
        print("it is shape:{}".format(I2.shape))
        print(I2)
        s2 =  time.time()
        ...
        intensity2Dv2 = np.reshape(I2, (N, N))
        ...
        # Creates PIL image
        img = Image.fromarray(np.uint8(intensity2Dv2 * 255) , 'L')
        img.save(filename2a)
        '''
